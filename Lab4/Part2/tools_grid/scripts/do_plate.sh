...
do
	...
	echo "------------------------- HCopy -------------------------------"
	HCopy -T 0 -C etc/config_code.mfcc data/test2/$PLATE.wav features/test2/$PLATE.mfcc

	echo "------------------------- HParse -------------------------------"
	HParse etc/grammar2 etc/wdnet2

	echo "------------------------- HVite -------------------------------"
	HVite -T 0 -C etc/config_train.mfcc -o ST -l '*' -H models/$MODELNAME/SI/hmm$MIX.d/newMacros -i labels/$PLATE.mlf -w etc/wdnet2 etc/dict2 etc/wdlist2 features/test2/$PLATE.mfcc

	echo "------------------------- Save results -------------------------------"
	plate=`expr substr $PLATE 5 6`
	RESFILE="$REC_ROOT/results/plateSI_${plate}_${MODELNAME}.txt"
